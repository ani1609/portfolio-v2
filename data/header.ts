import { NavItem } from '@/types/nav';

export const websiteLink: string = 'https://www.myselfankit.tech/';

export const navItems: NavItem[] = [
  { title: 'About', scrollTo: 'about-section' },
  { title: 'Experience', scrollTo: 'experience-section' },
  { title: 'Work', scrollTo: 'major-projects-section' },
  { title: 'Git Stats', scrollTo: 'github-footprints-section' },
  { title: 'Contact', scrollTo: 'contact-section' },
];

export const resumeLink: string =
  'https://drive.google.com/file/d/1YTaKaaGpWHLNf1vitduTPmzSq1VRwOb4/view?usp=sharing';

export const emailAddress: string = 'ankitparallax@gmail.com';
